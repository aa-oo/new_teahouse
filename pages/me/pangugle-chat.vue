			...
			<scroll-view class="msg-list" scroll-y="true" :scroll-with-animation="scrollAnimation" :scroll-top="scrollTop" :scroll-into-view="scrollToView" upper-threshold="50">
				...
				<view class="row" v-for="(row, index) in msglist" :key="index">
					...
					<block v-if="row.type == 2">
						<!-- <view class="system">
							<view class="text">{{ row.message }}</view>
						</view> -->
...
export default {
	...
	onLoad(option) {
		...
		if(this.mesdetail.type==1){
			if (this.mesdetail.userId == this.user.id) {
				console.log(1)
				this.getteamesid = this.mesdetail.toId;
			} else {
				console.log(2)
				this.getteamesid = this.mesdetail.userId;
			}
		}
		else{
			this.getteamesid = this.mesdetail.toId;
			console.log(this.getteamesid )
		}
		...
		this.socketTask.onMessage(res=>{
			...
			if(this.recmsglist.type==1){
				...
				if(this.recmsglist.fromId==this.user.id){
					console.log('是我 是我')
					console.log(this.recmsglist)
					// this.
					this.getteaMes()
