<template>
	<view class="content">
		<view class="logolog"><image class="logoImg" src="../../static/icon.png" /></view>
		<!-- 手机号登录 -->
		...
		<view class="nolog">
			<text @click="changstatus()">{{ status ? '账号密码登录' : '验证码登录' }}</text>
			<span style="padding:0 8px;">|</span>
			<span @click="subsign">登录遇到问题</span>
		</view>
		...
		<view class="nolog">
			<span>注册即代表同意</span>
			<span @click="subsign" style="color:#007AFF">《创客茶馆APP用户协议》</span>
		</view>
	</view>
</template>
...
export default {
	...
	computed: {
		disabled() {
			if ((this.usercode === '' || this.password === '') && (this.phone === '' || this.code === '')) {
				return true;
			}
			return false;
		}
	...
	methods: {
		...
		sublog() {
				...
				success: res => {
					...
					if (res.statusCode == 500) {
						...
						if (res.data.state == 200) {
							...
							console.log('个人user信息如下所示111');
							this.userid = res.data.obj.user.id;
							this.loginstate = true;
							console.log(res.data.obj.user);
							//持久化存储
							// 提示和跳转
							var socketTask = uni.connectSocket({
								url: 'ws://101.34.137.197/connection/' + this.userid,
								method: 'GET',
								complete: res => {
									console.log('1111');
									// this.$store.
							...
							this.$store.commit('SocketTask', socketTask);
							// sleep(1000)
							// console.log(JSON.stringify(this.try));
							// setTimeout(o => {
							// 	SocketTask.send({
							// 		data: JSON.stringify(this.try),
							// 		complete: res => {
			...
			// 	console.log("this.loginstate")
			// uni.connectSocket({
			// 	url:'ws://101.34.137.197/connection/'+'33',
			// 	// data() {
			// 	// 	return {
			// 	// 		x: '',
			// 	// 		y: ''
